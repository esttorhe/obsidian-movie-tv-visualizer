// ABOUTME: Actors route: cards for every cast member across movies and TV shows, sortable and searchable.
// ABOUTME: Clicking an actor opens their filmography via a cast-filtered catalog.
import { setIcon } from "obsidian";
import type { MediaItem, ActorCard } from "../types";
...
export function renderActors(container: HTMLElement, opts: ActorsViewOptions): void {
	...
	if (actors.length === 0) {
		...
		emptyP.textContent = "No actors found. Add a 'cast' field to your notes.";
	...
	const renderGrid = (query = "") => {
		...
		sorted.forEach((actor, i) => {
			...
			const parts: string[] = [`${actor.count} title${actor.count !== 1 ? "s" : ""}`];
...
function renderActorFilmography(
	...
	const catalogView = new CatalogView({
		...
		initialFilter: { mediaType: "all", genres: [], status: "all", cast: actor.name },
