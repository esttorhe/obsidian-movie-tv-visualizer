// ABOUTME: Playlists route: user-created lists spanning movies and TV shows, persisted in localStorage.
// ABOUTME: Create/open/delete playlists; a detail view renders the list's items as a card grid.
...
export function renderPlaylists(container: HTMLElement, opts: PlaylistViewOptions): void {
	...
	const renderList = () => {
		...
		if (playlists.length === 0) {
			...
			emptyP.textContent = "No playlists yet. Create one to organize your movies and shows.";
		...
		playlists.forEach((pl) => {
			...
			count.textContent = `${pl.itemIds.length} title${pl.itemIds.length !== 1 ? "s" : ""}`;
			...
			openBtn.addEventListener("click", () => renderPlaylistDetail(container, pl, opts));

			const deleteBtn = document.createElement("button");
			deleteBtn.className = "lmv-btn lmv-btn--ghost lmv-btn--danger";
			deleteBtn.textContent = "Delete";
			deleteBtn.addEventListener("click", () => {
				const all = loadPlaylists().filter((p) => p.id !== pl.id);
				savePlaylists(all);
				renderList();
			});
...
function renderPlaylistDetail(
	...
	if (items.length === 0) {
		...
		emptyP2.textContent = "No titles. Add them from a detail view.";
